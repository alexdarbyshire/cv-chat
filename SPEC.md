...
- **Account linking**: a guest's chat history migrates to their Google account on first sign-in (one-time, by guest-cookie session id). The migration happens inside the `jwt` callback — no separate UI.
- **Google `User` upsert lives in the `jwt` callback**, not in the Google provider's `profile()`. Auth.js v5 core overrides any `user.id` returned by `profile()` with `crypto.randomUUID()` before the `jwt` callback runs ([nextauthjs/next-auth#8377](https://github.com/nextauthjs/next-auth/issues/8377)), so the persisted `User.id` would be discarded if we resolved it there. We discriminate by `account.provider === "google"` inside `jwt` and call `getOrCreateGoogleUser` to produce the correct `token.id` that downstream `chat.userId` FKs target. Credentials/guest is unaffected — its `authorize()`-returned `user.id` reaches the `jwt` callback intact.
